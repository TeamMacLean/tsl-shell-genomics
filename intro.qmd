...
Our home directory is empty. Let's copy a directory called `shell_data` from the `dc_workshop` directory using the command `cp` for "**c**o**p**y". The flag `-r` stands for "recursive" and specifies that we want to copy the full content of the directory `shell_data`:

``` bash
$ cp -r /tsl/data/dc_workshop/shell_data/ /hpc-home/[username]/
```
...
``` bash
$ ls -F
metadata.csv*  SRA_reads/  untrimmed_fastq/
```
...
### Output
	...
	$ total 112
	$ -rwxrwx--- 1 taz23vul TSL_20 3797 Sep 22 12:21 metadata.csv
	$ drwxrwx--- 2 taz23vul TSL_20  667 Sep 22 12:22 SRA_reads
	$ drwxrwx--- 2 taz23vul TSL_20   33 Sep 22 12:36 untrimmed_fastq
